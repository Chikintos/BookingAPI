...
import { deleteEventCascase } from "../scripts/entetyDelete";
...
export const eventUpdate = asyncHandler(
  async (req: UserRequest, res: Response) => {
    ...
    try {
      const event = await eventRepository.findOne({ where:{id: event_id},relations:{venue:true} });
      const user = await userRepository.findOneBy({ id: req.user.id });
      if (venue_id) {
...
export const eventDelete = asyncHandler(
  async (req: UserRequest, res: Response) => {
    ...
    if (req.user.id!==Event.created_by.id && req.user.role !== UserRole.ADMIN){
      res.status(403)
...
export const eventGetByUser = asyncHandler(
  async (req: UserRequest, res: Response) => {
    ...
    const organizator : User = await userRepository.findOneBy({id:user_id})
    if (!organizator){
      res.status(404)
      throw new Error("user not found")
    }
...
export const eventAddPhoto = asyncHandler(
  async (req: UserRequest, res: Response) => {
    ...
    const Event = await eventRepository.findOne({where:{id:event_id},relations:{photos:true}})
    try{
      if (!Event){
        res.status(404)
        throw new Error("event not found")
...
export const eventGetPhoto = asyncHandler(
  async (req: UserRequest, res: Response) => {
    ...
    const photo_list = await eventPhotoRepository.find({where:{event}})    
  ...
  export const eventDeletePhoto = asyncHandler(
    async (req: UserRequest, res: Response) => {
      ...
      const Photo = await eventPhotoRepository.findOne({where:{id:eventPhoto_id},relations:['event', 'event.created_by']})
      if (req.user.role !== UserRole.ADMIN || req.user.id === Photo.event.created_by.id) {
        res.status(403);
