...
import {
  ...
  venueCreateSchema,
  venueUpdateSchema,
} from "../validators/venueValidator";
...
//handler to get venue information
...
//handler to update venue information
...
//handler to add a photo to a venue
...
//route handler to get photos of a venue
export const VenueGetPhoto = asyncHandler(
  async (req: UserRequest, res: Response) => {
    ...
    const photo_list = await venuePhotoRepository.find({
      where: { venue: Venue },
    });
...
//handler to delete a photo from a venue
export const VenueDeletePhoto = asyncHandler(
  async (req: UserRequest, res: Response) => {
    ...
    const Photo = await venuePhotoRepository.findOne({
      where: { id: venuePhoto_id },
    });
