...
export async function deleteVenueCascase (id : number) : Promise<void> {
    const venue : Venue = await venueRepository.findOne({where:{id},relations:{photos:true,events:true}})
...
export async function deleteEventCascase (id : number) : Promise<void> {
    const event = await eventRepository.findOne({where:{id},relations:{photos:true}})
